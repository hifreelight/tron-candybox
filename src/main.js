...
import monitorTronWeb from './util/web3/monitorTronWeb'
...
new Vue({
  ...
  beforeCreate: function () {
    ...
    this.$store.dispatch(ACTION_TYPES.REGISTER_TRONWEB_INSTANCE)
      .then((result) => {
        let state = result.state
        monitorTronWeb(state)
      })
      .catch((result = {}) => {
        let state = result.state
        monitorTronWeb(state)

        console.error(result, 'Unable to REGISTER_TRONWEB_INSTANCE')
      })
  },
  computed: {
    ...mapState({
  ...
  watch: {
    hasInjectedWeb3 (web3ConnectionValue) {
      ...
      console.log('window.web3 is %o', window.web3)
      console.log('web3 state is %o', this.$store.state)
    },
    hasInjectedTronWeb (tronWebConnectionValue) {
      console.log('hasInjectedTronWeb: ', tronWebConnectionValue)
      console.log('window.tronWeb is %o', window.tronWeb)
      console.log('tron state is %o', this.$store.state)
    },
    networkId (networkId) {
