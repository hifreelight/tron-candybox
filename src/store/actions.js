...
import getTronWeb from '../util/web3/getTronWeb'
...
export default {
  ...
  [ACTION_TYPES.REGISTER_TRONWEB_INSTANCE] ({ state, commit }) {
    return new Promise(function (resolve, reject) {
      getTronWeb
        .then((result) => {
          commit(MUTATION_TYPES.REGISTER_TRONWEB_INSTANCE, {
            result,
            callback: (state) => {
              resolve({ state })
            }
        ...
        .catch((error) => {
          if (!(state && state.tronWeb)) {
            const result = error.result
            commit(MUTATION_TYPES.REGISTER_TRONWEB_INSTANCE, {
              result: {
                tronWeb: (result && result.hasInjectedTronWeb ? result.tronWeb : null),
                hasInjectedTronWeb: (result && result.hasInjectedTronWeb ? result.hasInjectedTronWeb : false),
                tronWebError: error.error
              },
              callback: (state) => {
                reject({ state, error })
              }
